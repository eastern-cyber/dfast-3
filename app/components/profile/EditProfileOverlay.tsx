...
export default function EditProfileOverlay() {
    ...
    const validate = () => {
        ...
        if (!userName) {
            setError({ type: 'userName', message: 'ต้องระบุชื่อผู้ใช้งาน' })
            isError = true
        }
        return isError
    }
    ...
    const cropAndUpdateImage = async () => {
        ...
        try {
            if (!file) return alert('โปรดเลือกไฟล์ที่ต้องการอัพโหลด')
            if (!cropper) return alert('ไม่มีไฟล์ที่ต้องการ')
            setIsUpdating(true)
                    ...
                    <div className={`h-[calc(500px-200px)] ${!uploadedImage ? 'mt-16' : 'mt-[58px]'}`}>
                        {!uploadedImage ? (
                            <div>
                                    ...
                                    <div className="flex items-center justify-center sm:-mt-6">
                                        <div className="sm:w-[60%] w-full max-w-md">
                                            ...
                                            <p className={`relative text-[11px] text-gray-500 ${error ? 'mt-1' : 'mt-4'}`}>
                                                Usernammes can only contain letters, number, underscores, and periods.
                                                Changing your username will also change your profile link.
                                                ชื่อผู้ใช้ต้องประกอบด้วยตัวอักษร หมายเลข สัญลักษณ์ขีดล่าง และ จุด
                                                การเปลี่ยนชื่อผู้ใช้ จะมีผลให้ลิ้งค์ของโปรไฟล์เปลี่ยนไปด้วย
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                >
                                    <h3 className="font-semibold text-[15px] sm:mb-0 mb-1 text-gray-700">
                                        ข้อมูลส่วนตัวของผู้ใช้ User Bio
                                    </h3>
                                    <div className="flex items-center justify-center sm:-mt-6">
                                        <div className="sm:w-[60%] w-full max-w-md">
                                            <textarea
